...
function frame:COMBAT_LOG_EVENT_UNFILTERED()
	local _, eventType, _, _, _, sourceFlags, _, _, _, _, _, spellID = C_CombatLog.GetCurrentEventInfo()
	if eventType == "SPELL_CAST_SUCCESS" then
		if (bit.band(sourceFlags, Enum.CombatLogObject.TypePet) == Enum.CombatLogObject.TypePet and bit.band(sourceFlags, Enum.CombatLogObject.AffiliationMine) == Enum.CombatLogObject.AffiliationMine) then
