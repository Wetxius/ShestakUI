...
local function pull(_, elapsed)
	...
	if not firstdone then
		T.SendChatMessage(string.format(L_ANNOUNCE_PC_MSG, target, tostring(delay)), T.CheckChat(true))
		firstdone = true
		delay = delay - 1
	end
	...
	if lastupdate >= interval then
		lastupdate = 0
		if delay > 0 then
			T.SendChatMessage(tostring(delay).."..", T.CheckChat(true))
			delay = delay - 1
		else
			T.SendChatMessage(L_ANNOUNCE_PC_GO, T.CheckChat(true))
			reset()
		end
	end
end
...
function frame.Pull(timer)
	...
	if timerframe:GetScript("OnUpdate") then
		reset()
		T.SendChatMessage(L_ANNOUNCE_PC_ABORTED, T.CheckChat(true))
	else
