...
----------------------------------------------------------------------------------------
local DB
local SpellBinder = CreateFrame("Frame", "SpellBinder", UIParent, "ButtonFrameTemplate")
SpellBinder:Hide()
...
SpellBinder.makeSpellsList = function(_, delete)
	...
	for i, spell in ipairs(DB.spells) do
		...
		if v then
			...
			if C_Spell.GetSpellInfo(v) then
				bf:SetAlpha(1)
...
local function Load()
	SpellBinder:SetParent(PlayerSpellsFrame.SpellBookFrame)
	...
	SpellBinder.ToggleButtons = function()
		for i = 1, #SpellBinder.spellbuttons do
	...
	SpellBinder.OpenButton:SetScript("OnShow", function(self)
		...
		self:ClearAllPoints()
		self:SetPoint("TOPLEFT", PlayerSpellsFrame, "TOPRIGHT", 2, 0)
	...
	SpellBinder.OpenButton:SetScript("OnClick", function()
		...
		if SpellBinder:IsVisible() then
			...
			PlayerSpellsFrame.MaxMinButtonFrame.MinimizeButton:Click()
			SpellBinder:Show()
		...
		for _, frame in self:EnumerateFrames() do
			if frame.Button then
				if not frame.hook then
					local button = CreateFrame("Button", nil, frame.Button, "SecureActionButtonTemplate")
					...
					button:Hide()
					tinsert(SpellBinder.spellbuttons, button)
	...
	elseif C.skins.blizzard_frames == true then
		...
		SpellBinder.OpenButton:GetNormalTexture():SetTexCoord(0.1, 0.9, 0.1, 0.9)
		SpellBinder.OpenButton:SetPushedTexture("Interface\\ICONS\\Achievement_Guild_Doctorisin")
		SpellBinder.OpenButton:GetPushedTexture():SetTexCoord(0.1, 0.9, 0.1, 0.9)
		SpellBinder.OpenButton:GetPushedTexture():SetInside()
